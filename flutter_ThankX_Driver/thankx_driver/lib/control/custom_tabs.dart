...
class MyCustomTab extends StatelessWidget {
  ...
  const MyCustomTab({
    ...
    this.child,
        ...
        assert(!(text != null &&
            null !=
                child)), // TODO(goderbauer): https://github.com/dart-lang/sdk/issues/31140
        super(key: key);
...
class _IndicatorPainter extends CustomPainter {
  ...
  Rect indicatorRect(Size tabBarSize, int tabIndex) {
    ...
    return new Rect.fromLTWH(
        tabLeft, 0.0, tabRight - tabLeft, tabBarSize.height);
  }
...
class _TabBarState extends State<TabBar> {
  ...
  void didUpdateWidget(TabBar oldWidget) {
    ...
    if (widget.tabs.length > oldWidget.tabs.length) {
      ...
      _tabKeys.addAll(
          new List<GlobalKey>.generate(delta, (int n) => new GlobalKey()));
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.tabs.length; i += 1) {
      wrappedTabs[i] = Material(
        color: _controller.index == i ? widget.activeBackgroundColor : widget.inactiveBackgroundColor,
        child: new Center(
          heightFactor: 1.0,
    ...
    if (_controller != null) {
      ...
      if (_controller.indexIsChanging) {
        ...
        if (_currentIndex > 0) {
          ...
          new ReverseAnimation(new _DragAnimation(_controller, tabIndex));
          wrappedTabs[tabIndex] =
