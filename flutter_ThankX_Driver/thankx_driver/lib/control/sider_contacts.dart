...
class _SideBarState extends State<SideBar> {
  ...
  void initState() {
    ...
    _widgetList = List<Widget>.generate(widget.list.length, (index) {
      return Padding(
        ...
        child: Text(
          widget.list[index],
          style: TextStyle(color: widget.textColor, fontSize: 11,fontFamily: AppFont.sfProTextRegular)    ,
        ),
      );
    });
  }
  ...
  _onDragStart(BuildContext context, DragStartDetails start) {
    ...
    setState(() {
      _selectStr = _getCurrentStr(local.dy-UIUtills().getProportionalHeight(68));
...
//                borderRadius: BorderRadius.all(Radius.circular(15.0))
      ...
      child:Container(
              margin: EdgeInsets.only(top:UIUtills().getProportionalHeight(68)),
  ...
  String _getCurrentStr(double dy) {
    int index = (dy / UIUtills().getProportionalWidth(19)).round();
    if (index <= 0) {
